## Mysql
### Mysql组成
Mysql主要由Server层和存储引擎层组成，Server层又分为连接器、查询缓存、分析器、优化器、执行器
- 连接器：管理客户端连接，登录认证等
- 查询缓存：查询语句优先匹配查询缓存，若匹配成功则直接返回。更新语句会先在查询缓存中将更新涉及到的表的所有缓存都清空
- 分析器：通过词法分析和语法分析解析sql语句
- 优化器：根据sql看执行计划，选择最优执行计划（索引选择、join顺序选择等）
- 执行器：验权（用户对表的权限），执行执行计划，打开表，根据表的引擎定义向存储引擎调用接口操作
...
#### 最左匹配原则（索引顺序！！！）
从索引的最左边的列开始查找，当遇到范围查找时停止匹配（Mysql的查询优化器会针对where子句进行优化自动满足最左匹配原则）

#### 索引下推
索引下推是指当where语句中索引满足了最左匹配原则后还有其他条件也是联合索引中的字段时，mysql会继续过滤索引包含的字段，然后再返回主键索引来回表。
- 场景：索引A(a,b),where a like "A%" and b = 1，这个使用索引下推时，会在匹配了a条件后，继续过滤掉b的一部分内容再根据主键回表
...
### 锁
#### 元数据锁（MDL）
不需要显示设置，只要访问一个表时就会自动加上，避免在访问时表结构被改变。（增删改查表数据时MDL为读锁，修改表结构时MDL为写锁）

#### 行锁（row lock）和表锁（table lock）
只有当查询的字段是主键或者加了索引并且是指定匹配才触发行锁，其余情况除了未查询到结果不使用锁剩下的都是触发表锁。
- 两阶段锁协议：行锁会在需要的时候才加上，但需要在事务执行结束后才释放。
```
