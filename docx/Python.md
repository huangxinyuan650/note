...
#### 元类
1、拦截类的创建 2、并修改类的创建逻辑 3、最后返回修改后的类
...
- 共享内存：在内存中开辟一块对多个进程可见的空间，多个进程可读可写，但每次写入的都会覆盖上次内容。（Value、Array类，）
...
- 装饰器执行的时机：1、装饰器在模块加载的时候执行返回装饰好的函数，执行的是装饰好的函数。2、多个装饰器先自底向上执行装饰器外的内容逐层返回，然后被装饰函数在被调用的时候再从上往下执行（其实就是函数的嵌套）。
