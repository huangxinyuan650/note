...
##### Bit数组
Redis使用字符串表示位数组，每个字符8位，形成一个二维数组（字符是一个一维的数组，每个元素一个位），offset对应位置（行=offset//8,列=offset%8+1）
###### BitCount
计算bit数组中为1的个数
- 1、遍历方式
- 2、字典对照，计算出8位（或者16位等）的十进制表示以及1的个数形成字典
- 3、variable precision swar算法（先计算两位一组的1的个数，再计算4位一组的1的个数，再计算8位一组的1的个数，然后分别右移8位、16位、24位后相加得到结果的高8位中即为32位中所有1的个数，再左移24位即可）
```
def swar(i):
...
#### Redis序列化协议（RESP）
Redis客户端使用RESP协议与服务端进行通信
##### 特点：1、实现简单 2、解析快速 3、人类可读
##### 协议类型说明
Redis中响应类型往往通过第一个字符说明
- 1、简单字符串（+）2、错误（-）3、整数（:）4、批量字符串（$）5、数组（*）
